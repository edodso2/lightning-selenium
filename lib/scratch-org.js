...
/**
 * @typedef ScratchOrgOptions
 * @property {string} [deeplink]
 * @property {string} [alias]
 */

/**
 * @typedef ScratchOrgInitOptions
 * @property {boolean} [disableMultifactor] will disable mulitfactor by setting the
 * org's secure ip address range to the current executing environments ip address.
 */

/** @type {ScratchOrgOptions} */
...
class ScratchOrg {
  ...
  /**
   * @param {ScratchOrgOptions} [options] 
   */
  ...
  /**
   * Initialize the scratch org for testing.
   * @param {ScratchOrgInitOptions} [options] 
   */
  async init(options) {
    ...
    if (options && options.disableMultifactor) {
      try {
        await this._writeSecuritySettingsFile();
        console.warn('** WARNING ** disabling Multifactor will change the security setting of the org WITHOUT rolling back the changes.');
      } catch(error) {
